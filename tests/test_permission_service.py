...
def test_create_permission_without_name_raises_error(permission_service):
    with pytest.raises(PermissionValidationError, match=messages.PERMISSION_INVALID_NAME):
...
def test_update_permission_description_with_none(permission_service):
    ...
    with pytest.raises(PermissionValidationError, match=messages.PERMISSION_INVALID_TYPE):
